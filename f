...
-- Evita que una segunda ejecución deje conexiones y bucles de la anterior.
if _G.ROTX_HUB_CLEANUP then pcall(_G.ROTX_HUB_CLEANUP) end
if _G.ROTX_RUNTIME_CONNS then
    for _, connection in ipairs(_G.ROTX_RUNTIME_CONNS) do
        pcall(function() connection:Disconnect() end)
    end
end
_G.ROTX_RUNTIME_CONNS = {}
function trackRotxConnection(connection)
    table.insert(_G.ROTX_RUNTIME_CONNS, connection)
    return connection
end

-- ====== TEMA VISUAL MORADO ======
GUI_PURPLE = Color3.fromRGB(91, 33, 151)
GUI_PURPLE_DARK = Color3.fromRGB(27, 7, 46)
GUI_PURPLE_LIGHT = Color3.fromRGB(141, 72, 215)
GUI_MAGENTA = Color3.fromRGB(232, 62, 255)
GUI_CYAN = Color3.fromRGB(48, 221, 255)
GUI_BLUE = Color3.fromRGB(62, 96, 255)
GUI_BG = Color3.fromRGB(7, 3, 13)
GUI_PANEL = Color3.fromRGB(18, 7, 30)
GUI_PANEL_ALT = Color3.fromRGB(32, 11, 52)
GUI_PURPLE_DEEP = Color3.fromRGB(39, 11, 70)
GUI_MAGENTA_DEEP = Color3.fromRGB(78, 16, 91)
GUI_CYAN_DEEP = Color3.fromRGB(10, 52, 68)
GUI_WHITE = Color3.fromRGB(255, 255, 255)
GUI_BLACK = Color3.fromRGB(0, 0, 0)
local NS, CS = 60, 29
...
local function startAutoSteal()
    ...
    local elapsed = 0
    Conns.autoSteal = RunService.Heartbeat:Connect(function(dt)
            ...
            if char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChildOfClass("Humanoid") and char:FindFirstChildOfClass("Humanoid").Health > 0 then
                ...
                if not label then
                    ...
                    if head then
                        ...
                        textLabel.TextColor3 = GUI_WHITE
...
local function startEnemySpeed()
    ...
    local elapsed = 0
    enemySpeedConn = RunService.Heartbeat:Connect(function(dt)
        elapsed = elapsed + dt
...
-- Núcleo de movimiento: conserva la respuesta, pero evita recorrer a todos
-- los jugadores y actualizar textos en cada fotograma.
function startCoreMovementLoops()
    if steppedConn then steppedConn:Disconnect(); steppedConn = nil end
    if movementLoop then movementLoop:Disconnect(); movementLoop = nil end

    local collisionElapsed = 0
    steppedConn = RunService.Heartbeat:Connect(function(dt)
        collisionElapsed = collisionElapsed + dt
        if collisionElapsed < 0.25 then return end
        collisionElapsed = 0
    ...
    movementLoop = RunService.RenderStepped:Connect(function(dt)
        ...
        speedElapsed = speedElapsed + dt
        if speedLabel and speedElapsed >= 0.1 then
            speedElapsed = 0
...
local function setupSpeedIndicator(char)
    ...
    local oldBB = head:FindFirstChild("RotxHubSpeedIndicator")
    if oldBB then oldBB:Destroy() end
    local bb = Instance.new("BillboardGui", head)
    bb.Name = "RotxHubSpeedIndicator"
    ...
    titleLabel.Text = "Rotx Hub"
    titleLabel.TextColor3 = GUI_WHITE
    ...
    speedLabel.Text = "0.0"
    speedLabel.TextColor3 = GUI_WHITE
...
trackRotxConnection(RunService.Heartbeat:Connect(function()
    if not jumpEnabled then return end
    local char = LP.Character
    if not char then return end
    local root = char:FindFirstChild("HumanoidRootPart")
...
enableAutoBat = function()
    ...
    if bypassToggled then
        ...
        if bypassFloatingButton then
            ...
            if btnFrame then
                btnFrame.BackgroundColor3 = GUI_PURPLE_DARK
                local lbl = btnFrame:FindFirstChild("TextLabel")
                if lbl then lbl.TextColor3 = GUI_WHITE end
            end
        end
        stopBypassAimbot()
    end
...
local function toggleBypass(state)
    ...
    if bypassFloatingButton then
        ...
        if btnFrame then
            ...
            if bypassToggled then
                btnFrame.BackgroundColor3 = GUI_PURPLE_LIGHT
                if label then label.TextColor3 = GUI_WHITE end
            else
                btnFrame.BackgroundColor3 = GUI_PURPLE_DARK
                if label then label.TextColor3 = GUI_WHITE end
            end
...
-- ====== CONFIGURACIÓN ======
local CONFIG_FILE = "RotxHub.json"
...
local function deleteAllSettings()
    ...
    if isfile and isfile("RotxHub_PanelPos.txt") then
        pcall(delfile, "RotxHub_PanelPos.txt")
    end
    resetToDefaults()
    if pbFrame then
...
-- ====== ANIMACIÓN DE ENTRADA ROTX HUB ======
function playRotxHubIntro()
    local oldIntro = game:GetService("CoreGui"):FindFirstChild("RotxHubIntro")
    if oldIntro then oldIntro:Destroy() end

    local intro = Instance.new("ScreenGui")
    intro.Name = "RotxHubIntro"
    intro.ResetOnSpawn = false
    intro.IgnoreGuiInset = true
    intro.DisplayOrder = 999
    intro.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    pcall(function() if syn and syn.protect_gui then syn.protect_gui(intro) end end)
    if not pcall(function() intro.Parent = game:GetService("CoreGui") end) then
        intro.Parent = LP:WaitForChild("PlayerGui")
    end

    local backdrop = Instance.new("Frame", intro)
    backdrop.Size = UDim2.new(1, 0, 1, 0)
    backdrop.BackgroundColor3 = GUI_BLACK
    backdrop.BackgroundTransparency = 1
    backdrop.BorderSizePixel = 0
    backdrop.ZIndex = 0

    local flash = Instance.new("Frame", intro)
    flash.Size = UDim2.new(1, 0, 1, 0)
    flash.BackgroundColor3 = GUI_PURPLE_LIGHT
    ...
    aura.BackgroundColor3 = GUI_PURPLE_LIGHT
    aura.BackgroundTransparency = 0.72
    ...
    TS:Create(aura, TweenInfo.new(0.55, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        Size = UDim2.new(0, 430, 0, 185),
        BackgroundTransparency = 0.88
    }):Play()

    local card = Instance.new("Frame", intro)
    card.AnchorPoint = Vector2.new(0.5, 0.5)
    card.Position = UDim2.new(0.5, 0, 0.5, 0)
    card.Size = UDim2.new(0, 0, 0, 118)
    card.BackgroundColor3 = GUI_PURPLE_DARK
    ...
    cardScale.Scale = 0.78

    local cardStroke = Instance.new("UIStroke", card)
    cardStroke.Color = GUI_BLACK
    cardStroke.Thickness = 2
    cardStroke.Transparency = 0

    local glow = Instance.new("Frame", card)
    glow.Size = UDim2.new(1, 0, 1, 0)
    glow.BackgroundColor3 = GUI_PURPLE
    glow.BackgroundTransparency = 0.45
    glow.BorderSizePixel = 0
    local glowGrad = Instance.new("UIGradient", glow)
    glowGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, GUI_PURPLE_DARK),
        ColorSequenceKeypoint.new(0.5, GUI_PURPLE_LIGHT),
        ColorSequenceKeypoint.new(1, GUI_PURPLE_DARK)
    })
    glowGrad.Rotation = 25

    local scanLine = Instance.new("Frame", card)
    scanLine.Size = UDim2.new(1, 0, 0, 3)
    scanLine.Position = UDim2.new(0, 0, 0, -4)
    scanLine.BackgroundColor3 = GUI_WHITE
    scanLine.BackgroundTransparency = 0.18
    scanLine.BorderSizePixel = 0
    scanLine.ZIndex = 10
    local scanGlow = Instance.new("UIStroke", scanLine)
    scanGlow.Color = GUI_PURPLE_LIGHT
    scanGlow.Thickness = 4
    scanGlow.Transparency = 0.45

    local topCode = Instance.new("TextLabel", card)
    topCode.Size = UDim2.new(1, -24, 0, 14)
    topCode.Position = UDim2.new(0, 12, 0, 5)
    topCode.BackgroundTransparency = 1
    topCode.Text = "ROTX://SECURE_BOOT_SEQUENCE"
    topCode.TextColor3 = GUI_WHITE
    topCode.TextTransparency = 0.35
    topCode.Font = Enum.Font.Code
    topCode.TextSize = 8
    topCode.TextXAlignment = Enum.TextXAlignment.Left
    topCode.ZIndex = 8

    local glitchBack = Instance.new("TextLabel", card)
    glitchBack.Size = UDim2.new(1, 0, 0, 58)
    glitchBack.Position = UDim2.new(0, 3, 0, 18)
    glitchBack.BackgroundTransparency = 1
    glitchBack.Text = ""
    glitchBack.TextColor3 = GUI_BLACK
    glitchBack.TextTransparency = 0.25
    glitchBack.Font = Enum.Font.GothamBlack
    glitchBack.TextSize = 38
    glitchBack.ZIndex = 2

    local glitchLight = Instance.new("TextLabel", card)
    glitchLight.Size = UDim2.new(1, 0, 0, 58)
    glitchLight.Position = UDim2.new(0, -3, 0, 18)
    glitchLight.BackgroundTransparency = 1
    glitchLight.Text = ""
    glitchLight.TextColor3 = GUI_PURPLE_LIGHT
    glitchLight.TextTransparency = 0.2
    glitchLight.Font = Enum.Font.GothamBlack
    glitchLight.TextSize = 38
    glitchLight.ZIndex = 3
    ...
    track.Size = UDim2.new(0, 268, 0, 3)
    track.Position = UDim2.new(0.5, -134, 1, -17)
    track.BackgroundColor3 = GUI_BLACK
    ...
    fill.BackgroundColor3 = GUI_WHITE
    ...
    fillGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, GUI_WHITE),
        ColorSequenceKeypoint.new(0.5, GUI_PURPLE_LIGHT),
        ColorSequenceKeypoint.new(1, GUI_WHITE)
    })

    pcall(function()
        game:GetService("ContentProvider"):PreloadAsync({introSound})
    end)

    for i = 1, 12 do
        local particle = Instance.new("Frame", intro)
        local particleSize = math.random(2, 6)
        particle.Size = UDim2.new(0, particleSize, 0, particleSize)
        particle.AnchorPoint = Vector2.new(0.5, 0.5)
        particle.Position = UDim2.new(0.5, math.random(-45, 45), 0.5, math.random(-25, 25))
        particle.BackgroundColor3 = (i % 3 == 0) and GUI_WHITE or GUI_PURPLE_LIGHT
        particle.BackgroundTransparency = 0.15
        particle.BorderSizePixel = 0
        particle.Rotation = math.random(0, 90)
        particle.ZIndex = 3
        Instance.new("UICorner", particle).CornerRadius = UDim.new(1, 0)
        TS:Create(particle, TweenInfo.new(math.random(65, 120) / 100, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Position = UDim2.new(0.5, math.random(-260, 260), 0.5, math.random(-150, 150)),
            BackgroundTransparency = 1,
            Rotation = particle.Rotation + math.random(90, 270)
        }):Play()
    end

    introSound:Play()
    local audioPulseConn
    local audioPulseElapsed = 0
    audioPulseConn = RunService.RenderStepped:Connect(function(dt)
        if not introSound or not introSound.Parent then
            if audioPulseConn then audioPulseConn:Disconnect(); audioPulseConn = nil end
            return
        end
        audioPulseElapsed = audioPulseElapsed + dt
        if audioPulseElapsed < 0.05 then return end
        audioPulseElapsed = 0
        local pulse = math.clamp(introSound.PlaybackLoudness / 550, 0, 1)
        aura.BackgroundTransparency = 0.94 - (pulse * 0.2)
        glow.BackgroundTransparency = 0.52 - (pulse * 0.25)
        cardStroke.Thickness = 2 + (pulse * 3)
    end)

    TS:Create(card, TweenInfo.new(0.38, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Size = UDim2.new(0, 348, 0, 118)
    }):Play()
    TS:Create(cardScale, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1}):Play()
    TS:Create(scanLine, TweenInfo.new(0.72, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, 2), {
        Position = UDim2.new(0, 0, 1, 3)
    }):Play()
    task.wait(0.4)

    local logoText = "ROTX HUB"
    for i = 1, #logoText do
        local visibleText = string.sub(logoText, 1, i)
        title.Text = visibleText
        glitchBack.Text = visibleText
        glitchLight.Text = visibleText
        glitchBack.Position = UDim2.new(0, math.random(-5, 5), 0, 18 + math.random(-2, 2))
        glitchLight.Position = UDim2.new(0, math.random(-5, 5), 0, 18 + math.random(-2, 2))
        glitchBack.TextTransparency = 0.05
        glitchLight.TextTransparency = 0.05
        fill.Size = UDim2.new(i / #logoText, 0, 1, 0)
        titleScale.Scale = 1.08
        TS:Create(titleScale, TweenInfo.new(0.1, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1}):Play()
        glowGrad.Rotation = glowGrad.Rotation + 19
        task.wait(0.075)
        glitchBack.TextTransparency = 0.45
        glitchLight.TextTransparency = 0.35
    end

    sub.Text = "SYSTEM ONLINE  //  ACCESS GRANTED"
    TS:Create(sub, TweenInfo.new(0.2), {TextTransparency = 0}):Play()
    TS:Create(flash, TweenInfo.new(0.07), {BackgroundTransparency = 0.72}):Play()
    ...
    for i = 1, 14 do
        glitchBack.Position = UDim2.new(0, math.random(-7, 7), 0, 18 + math.random(-3, 3))
        glitchLight.Position = UDim2.new(0, math.random(-7, 7), 0, 18 + math.random(-3, 3))
        glitchBack.TextTransparency = (i % 2 == 0) and 0.15 or 0.7
        glitchLight.TextTransparency = (i % 2 == 0) and 0.65 or 0.12
        title.Position = UDim2.new(0, math.random(-2, 2), 0, 18)
        card.Rotation = (i % 2 == 0) and 0.35 or -0.35
        task.wait(0.045)
    end
    card.Rotation = 0
    title.Position = UDim2.new(0, 0, 0, 18)
    glitchBack.Position = UDim2.new(0, 2, 0, 18)
    glitchLight.Position = UDim2.new(0, -2, 0, 18)
    glitchBack.TextTransparency = 0.55
    glitchLight.TextTransparency = 0.55
    TS:Create(aura, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Size = UDim2.new(0, 500, 0, 220),
        BackgroundTransparency = 0.94
    }):Play()
    task.wait(0.5)

    TS:Create(title, TweenInfo.new(0.22), {TextTransparency = 1, TextStrokeTransparency = 1}):Play()
    TS:Create(glitchBack, TweenInfo.new(0.18), {TextTransparency = 1}):Play()
    TS:Create(glitchLight, TweenInfo.new(0.18), {TextTransparency = 1}):Play()
    TS:Create(sub, TweenInfo.new(0.18), {TextTransparency = 1}):Play()
    TS:Create(fill, TweenInfo.new(0.18), {BackgroundTransparency = 1}):Play()
    TS:Create(track, TweenInfo.new(0.18), {BackgroundTransparency = 1}):Play()
    TS:Create(cardStroke, TweenInfo.new(0.22), {Transparency = 1}):Play()
    TS:Create(backdrop, TweenInfo.new(0.28), {BackgroundTransparency = 1}):Play()
    TS:Create(aura, TweenInfo.new(0.25), {Size = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 1}):Play()
    TS:Create(introSound, TweenInfo.new(0.28), {Volume = 0}):Play()
    TS:Create(card, TweenInfo.new(0.28, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
        Size = UDim2.new(0, 0, 0, 118),
        BackgroundTransparency = 1
    }):Play()
    task.wait(0.3)
    if audioPulseConn then audioPulseConn:Disconnect(); audioPulseConn = nil end
...
local function applyShimmerToText(obj, speed)
    if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
        obj.TextColor3 = GUI_WHITE
        obj.TextStrokeColor3 = GUI_BLACK
        obj.TextStrokeTransparency = 0.16
    end
    return obj
end

-- Arte Y2K/cybercore del menu. La comprobacion es finita para no dejar
-- procesos visuales activos ni consumir recursos despues de abrir la GUI.
function loadRotxMenuArtwork(imageLabel)
    task.spawn(function()
        local contentProvider = game:GetService("ContentProvider")
        local candidates = {
            "rbxassetid://15876453323",
            "rbxthumb://type=Asset&id=15876453323&w=420&h=420",
            "rbxassetid://116267636440733",
            "rbxthumb://type=Asset&id=116267636440733&w=420&h=420",
            "rbxassetid://15876456117",
            "rbxthumb://type=Asset&id=15876456117&w=420&h=420",
            "rbxassetid://476778305"
        }
        ...
        for _, source in ipairs(candidates) do
            ...
            local loaded = false
            pcall(function() loaded = imageLabel.IsLoaded end)
            if loaded then return end
        end
    end)
end

function createRotxCyberBadge(parent)
    local badge = Instance.new("ImageLabel", parent)
    badge.Name = "CyberBadge"
    badge.Size = UDim2.new(0, 20, 0, 20)
    badge.Position = UDim2.new(1, -24, 0, 11)
    badge.BackgroundColor3 = GUI_BG
    badge.BackgroundTransparency = 0.04
    badge.BorderSizePixel = 0
    badge.Image = "rbxthumb://type=Asset&id=15876456117&w=150&h=150"
    badge.ImageColor3 = Color3.fromRGB(235, 232, 255)
    badge.ImageTransparency = 0.04
    badge.ScaleType = Enum.ScaleType.Crop
    badge.ZIndex = 21
    Instance.new("UICorner", badge).CornerRadius = UDim.new(0, 6)

    local stroke = Instance.new("UIStroke", badge)
    stroke.Color = GUI_BLACK
    stroke.Thickness = 1.2
    stroke.Transparency = 0.05
end

function createRotxDidacticHeader(parent)
    local header = Instance.new("Frame", parent)
    header.Name = "DidacticHeader"
    header.Size = UDim2.new(1, -48, 0, 34)
    header.Position = UDim2.new(0, 8, 0, 9)
    header.BackgroundColor3 = GUI_BG
    ...
    title.Name = "Title"
    title.Size = UDim2.new(1, -48, 0, 14)
    ...
    subtitle.Position = UDim2.new(0, 9, 0, 18)
    subtitle.BackgroundTransparency = 1
    subtitle.Text = "Movimiento y combate"
    subtitle.TextColor3 = Color3.fromRGB(225, 220, 245)
    subtitle.Font = Enum.Font.GothamMedium
    subtitle.TextSize = 7
    subtitle.TextXAlignment = Enum.TextXAlignment.Left
    subtitle.TextStrokeColor3 = GUI_BLACK
    subtitle.TextStrokeTransparency = 0.2
    subtitle.ZIndex = 6

    local index = Instance.new("TextLabel", header)
    index.Name = "Index"
    index.Size = UDim2.new(0, 24, 0, 24)
    index.Position = UDim2.new(1, -29, 0.5, -12)
    index.BackgroundColor3 = GUI_PURPLE_DEEP
    index.BackgroundTransparency = 0.04
    index.BorderSizePixel = 0
    index.Text = "01"
    index.TextColor3 = GUI_WHITE
    index.Font = Enum.Font.GothamBlack
    index.TextSize = 9
    index.TextStrokeColor3 = GUI_BLACK
    index.TextStrokeTransparency = 0.08
    index.ZIndex = 6
    Instance.new("UICorner", index).CornerRadius = UDim.new(0, 7)
    local indexStroke = Instance.new("UIStroke", index)
    indexStroke.Color = GUI_BLACK
    indexStroke.Thickness = 1
end

function updateRotxDidacticHeader(parent, pageName)
    local header = parent and parent:FindFirstChild("DidacticHeader")
    if not header then return end
    local title = header:FindFirstChild("Title")
    local subtitle = header:FindFirstChild("Subtitle")
    local index = header:FindFirstChild("Index")

    if pageName == "OtherPage" then
        if title then title.Text = "OTHER TOOLS" end
        if subtitle then subtitle.Text = "Utilidades y teletransporte" end
        if index then index.Text = "02" end
    elseif pageName == "ConfigPage" then
        if title then title.Text = "CONFIGURATION" end
        if subtitle then subtitle.Text = "Interfaz y valores" end
        if index then index.Text = "03" end
    elseif pageName == "KeybindsPage" then
        if title then title.Text = "KEYBINDS" end
        if subtitle then subtitle.Text = "Controles y atajos" end
        if index then index.Text = "04" end
    else
        if title then title.Text = "MAIN CONTROL" end
        if subtitle then subtitle.Text = "Movimiento y combate" end
        if index then index.Text = "01" end
    end
end

function decorateRotxMiniButton(button)
    button.Size = UDim2.new(0, 142, 0, 46)
    button.Position = UDim2.new(0, 16, 0, 58)
    button.BackgroundColor3 = GUI_BG
    ...
    local blackStroke = Instance.new("UIStroke", button)
    blackStroke.Color = GUI_BLACK
    blackStroke.Thickness = 2
    blackStroke.Transparency = 0.05

    local glowStroke = Instance.new("UIStroke", button)
    glowStroke.Color = GUI_CYAN
    glowStroke.Thickness = 3.5
    glowStroke.Transparency = 0.36

    local surfaceGrad = Instance.new("UIGradient", button)
    surfaceGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, GUI_BG),
        ColorSequenceKeypoint.new(0.48, GUI_PURPLE_DEEP),
        ColorSequenceKeypoint.new(0.78, GUI_MAGENTA_DEEP),
        ColorSequenceKeypoint.new(1, GUI_BG)
    })
    surfaceGrad.Rotation = 20

    local buttonScale = Instance.new("UIScale", button)
    buttonScale.Scale = 1

    local emblem = Instance.new("Frame", button)
    emblem.Size = UDim2.new(0, 27, 0, 27)
    emblem.Position = UDim2.new(0, 9, 0.5, -13)
    emblem.BackgroundColor3 = GUI_PURPLE_DEEP
    emblem.BorderSizePixel = 0
    emblem.ZIndex = 22
    Instance.new("UICorner", emblem).CornerRadius = UDim.new(1, 0)
    local emblemStroke = Instance.new("UIStroke", emblem)
    emblemStroke.Color = GUI_BLACK
    emblemStroke.Thickness = 1.5
    local emblemGlow = Instance.new("UIStroke", emblem)
    emblemGlow.Color = GUI_MAGENTA
    emblemGlow.Thickness = 2.6
    emblemGlow.Transparency = 0.32

    local emblemText = Instance.new("TextLabel", emblem)
    emblemText.Size = UDim2.new(1, 0, 1, 0)
    emblemText.BackgroundTransparency = 1
    emblemText.Text = "R"
    emblemText.TextColor3 = GUI_WHITE
    emblemText.Font = Enum.Font.GothamBlack
    emblemText.TextSize = 15
    emblemText.ZIndex = 23
    applyShimmerToText(emblemText, 1.2)

    local title = Instance.new("TextLabel", button)
    title.Size = UDim2.new(0, 96, 0, 18)
    ...
    title.TextXAlignment = Enum.TextXAlignment.Left
    title.ZIndex = 22
    applyShimmerToText(title, 1)

    local subtitle = Instance.new("TextLabel", button)
    subtitle.Size = UDim2.new(0, 96, 0, 12)
    subtitle.Position = UDim2.new(0, 40, 0, 25)
    subtitle.BackgroundTransparency = 1
    subtitle.Text = "CONTROL PANEL"
    subtitle.TextColor3 = GUI_WHITE
    subtitle.Font = Enum.Font.GothamBold
    subtitle.TextSize = 7
    subtitle.TextTransparency = 0.08
    subtitle.TextXAlignment = Enum.TextXAlignment.Left
    subtitle.ZIndex = 22
    applyShimmerToText(subtitle, 0.65)

    local shine = Instance.new("Frame", button)
    shine.Size = UDim2.new(0, 24, 0, 76)
    shine.Position = UDim2.new(0, -52, 0.5, -38)
    shine.BackgroundColor3 = GUI_WHITE
    ...
    button.MouseEnter:Connect(function()
        shine.Position = UDim2.new(0, -52, 0.5, -38)
        TS:Create(shine, TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Position = UDim2.new(0, 168, 0.5, -38)
        }):Play()
        TS:Create(buttonScale, TweenInfo.new(0.18, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1.08}):Play()
        TS:Create(button, TweenInfo.new(0.18), {BackgroundColor3 = GUI_PANEL_ALT}):Play()
        TS:Create(glowStroke, TweenInfo.new(0.18), {Transparency = 0.12, Thickness = 4.5}):Play()
    end)
    button.MouseLeave:Connect(function()
        TS:Create(buttonScale, TweenInfo.new(0.18, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1}):Play()
        TS:Create(button, TweenInfo.new(0.18), {BackgroundColor3 = GUI_BG}):Play()
        TS:Create(glowStroke, TweenInfo.new(0.18), {Transparency = 0.36, Thickness = 3.5}):Play()
    end)
...
local function buildGui()
    local BLACK   = GUI_BG
    local ACCENT  = GUI_WHITE
    local INP     = GUI_PANEL
    local CORNER  = 20
    local GUI_W, GUI_H = 345, 385
    local SIDEBAR_W = 128
    local CONTENT_OVERLAP = 6

    local old=game:GetService("CoreGui"):FindFirstChild("RotxHub");if old then old:Destroy() end
    local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild("RotxHub");if o then o:Destroy() end end
    gui=Instance.new("ScreenGui")
    gui.Name="RotxHub";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true
    ...
    mainGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, GUI_BG),
        ColorSequenceKeypoint.new(0.28, GUI_PURPLE_DEEP),
        ColorSequenceKeypoint.new(0.55, GUI_MAGENTA_DEEP),
        ColorSequenceKeypoint.new(0.78, GUI_PURPLE_DEEP),
        ColorSequenceKeypoint.new(1, GUI_CYAN_DEEP)
    })
    mainGrad.Rotation = 45
    mainGrad.Offset = Vector2.new(0,0)
    main.BackgroundColor3 = GUI_BG
    ...
    mainStroke.Color=GUI_BLACK
    mainStroke.Thickness=1.2
    mainStroke.Transparency=0.55

    local epicGlowStroke = Instance.new("UIStroke", main)
    epicGlowStroke.Color = GUI_CYAN
    epicGlowStroke.Thickness = 3
    epicGlowStroke.Transparency = 0.72
    ...
    local statusPill = Instance.new("Frame", sidebar)
    statusPill.Size = UDim2.new(0, 91, 0, 20)
    statusPill.Position = UDim2.new(0, 12, 0, 11)
    statusPill.BackgroundColor3 = GUI_PANEL
    statusPill.BackgroundTransparency = 0.05
    statusPill.BorderSizePixel = 0
    statusPill.ZIndex = 20
    Instance.new("UICorner", statusPill).CornerRadius = UDim.new(1, 0)
    local statusStroke = Instance.new("UIStroke", statusPill)
    statusStroke.Color = GUI_BLACK
    statusStroke.Thickness = 1
    local statusText = Instance.new("TextLabel", statusPill)
    statusText.Size = UDim2.new(1, 0, 1, 0)
    statusText.BackgroundTransparency = 1
    statusText.Text = "●  ROTX ONLINE"
    statusText.TextColor3 = GUI_WHITE
    statusText.Font = Enum.Font.GothamBold
    statusText.TextSize = 8
    statusText.TextStrokeColor3 = GUI_CYAN
    statusText.TextStrokeTransparency = 0.44
    statusText.ZIndex = 21
    local statusGrad = Instance.new("UIGradient", statusText)
    statusGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, GUI_CYAN),
        ColorSequenceKeypoint.new(0.35, GUI_WHITE),
        ColorSequenceKeypoint.new(1, GUI_MAGENTA)
    })
    statusText.TextStrokeColor3 = GUI_BLACK
    statusText.TextStrokeTransparency = 0.18
    createRotxCyberBadge(sidebar)

    local sidebarCanvas = Instance.new("Frame", sidebar)
    sidebarCanvas.Size = UDim2.new(1, 0, 1, 0)
    sidebarCanvas.Position = UDim2.new(0, 0, 0, 0)
    sidebarCanvas.BackgroundColor3 = GUI_BG
    sidebarCanvas.BackgroundTransparency = 0.48
    ...
    local sidebarImg = Instance.new("ImageLabel", main)
    sidebarImg.Size = UDim2.new(1, 0, 1, 0)
    sidebarImg.Position = UDim2.new(0, 0, 0, 0)
    sidebarImg.BackgroundTransparency = 1
    sidebarImg.Image = "rbxassetid://15876453323"
    sidebarImg.ImageColor3 = Color3.fromRGB(225, 222, 255)
    sidebarImg.ImageTransparency = 0.04
    sidebarImg.ScaleType = Enum.ScaleType.Crop
    sidebarImg.ClipsDescendants = true
    sidebarImg.ZIndex = 0

    loadRotxMenuArtwork(sidebarImg)

    local sidebarScrim = Instance.new("Frame", main)
    sidebarScrim.Size = UDim2.new(1, 0, 1, 0)
    sidebarScrim.BackgroundColor3 = GUI_BG
    sidebarScrim.BackgroundTransparency = 0
    sidebarScrim.BorderSizePixel = 0
    sidebarScrim.ClipsDescendants = true
    sidebarScrim.ZIndex = 0
    local sidebarShade = Instance.new("UIGradient", sidebarScrim)
    sidebarShade.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, GUI_BG),
        ColorSequenceKeypoint.new(0.48, GUI_PURPLE_DEEP),
        ColorSequenceKeypoint.new(1, GUI_BG)
    })
    sidebarShade.Transparency = NumberSequence.new({
        NumberSequenceKeypoint.new(0, 0.18),
        NumberSequenceKeypoint.new(0.5, 0.5),
        NumberSequenceKeypoint.new(1, 0.24)
    })
    sidebarShade.Rotation = 90

    -- Tab container
    local tabContainer = Instance.new("Frame", sidebar)
    tabContainer.Size = UDim2.new(0, 84, 0, 218)
    tabContainer.AnchorPoint = Vector2.new(0.5, 0.5)
    tabContainer.Position = UDim2.new(0.50, 0, 0.5, 0)
    ...
    brandFrame.Position = UDim2.new(0, 10, 1, -53)
    ...
    brandTitle.Text = "Rotx Hub"
    brandTitle.TextColor3 = GUI_WHITE
    brandTitle.Font = Enum.Font.GothamBold
    brandTitle.TextSize = 13
    brandTitle.TextXAlignment = Enum.TextXAlignment.Left
    brandTitle.TextStrokeColor3 = GUI_CYAN
    brandTitle.TextStrokeTransparency = 0.42
    local titleGrad = Instance.new("UIGradient", brandTitle)
    titleGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, GUI_CYAN),
        ColorSequenceKeypoint.new(0.5, GUI_WHITE),
        ColorSequenceKeypoint.new(1, GUI_MAGENTA)
    })
    titleGrad.Offset = Vector2.new(0,0)
    brandTitle.TextStrokeColor3 = GUI_BLACK
    brandTitle.TextStrokeTransparency = 0.12
    ...
    brandSub.Text = "By Alexelmascapito200"
    brandSub.TextColor3 = GUI_WHITE
    ...
    brandLine.Size = UDim2.new(0, 78, 0, 2)
    brandLine.Position = UDim2.new(0, 0, 0, 24)
    brandLine.BackgroundColor3 = GUI_WHITE
    brandLine.BorderSizePixel = 0
    Instance.new("UICorner", brandLine).CornerRadius = UDim.new(1, 0)
    local brandLineGrad = Instance.new("UIGradient", brandLine)
    ...
    closeBtn.BackgroundColor3 = GUI_PANEL
    closeBtn.BackgroundTransparency = 0
    closeBtn.BorderSizePixel = 0
    closeBtn.Text = "-"
    closeBtn.TextColor3 = GUI_WHITE
    ...
    closeStroke.Color = GUI_BLACK
    closeStroke.Thickness = 1.2
    closeStroke.Transparency = 0.3
    applyShimmerToText(closeBtn, 1.2)

    closeBtn.MouseEnter:Connect(function()
        TS:Create(closeBtn,TweenInfo.new(0.1),{TextColor3=GUI_WHITE,BackgroundColor3=GUI_PURPLE}):Play()
        TS:Create(closeStroke,TweenInfo.new(0.1),{Transparency=0,Color=GUI_BLACK}):Play()
    end)
    closeBtn.MouseLeave:Connect(function()
        TS:Create(closeBtn,TweenInfo.new(0.1),{TextColor3=GUI_WHITE,BackgroundColor3=GUI_PANEL}):Play()
        TS:Create(closeStroke,TweenInfo.new(0.1),{Transparency=0.3,Color=GUI_BLACK}):Play()
    end)

    miniBtn=Instance.new("TextButton",gui)
    miniBtn.Visible=false
    decorateRotxMiniButton(miniBtn)
    ...
    contentArea.BackgroundColor3 = GUI_BG
    contentArea.BackgroundTransparency = 0.28
    contentArea.BorderSizePixel = 0
    contentArea.ClipsDescendants = true
    Instance.new("UICorner", contentArea).CornerRadius = UDim.new(0, 26)
    local contentGrad = Instance.new("UIGradient", contentArea)
    contentGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, GUI_BG),
        ColorSequenceKeypoint.new(0.38, GUI_PURPLE_DEEP),
        ColorSequenceKeypoint.new(0.68, GUI_MAGENTA_DEEP),
        ColorSequenceKeypoint.new(1, GUI_CYAN_DEEP)
    })
    contentGrad.Transparency = NumberSequence.new(0.18)
    contentGrad.Rotation = 120
    local contentSt=Instance.new("UIStroke",contentArea)
    contentSt.Color=GUI_BLACK; contentSt.Thickness=1; contentSt.Transparency=0.18

    local contentAccent = Instance.new("Frame", contentArea)
    contentAccent.Size = UDim2.new(0, 70, 0, 2)
    contentAccent.Position = UDim2.new(0, 8, 0, 5)
    contentAccent.BackgroundColor3 = GUI_WHITE
    contentAccent.BackgroundTransparency = 0.18
    contentAccent.BorderSizePixel = 0
    contentAccent.ZIndex = 3
    Instance.new("UICorner", contentAccent).CornerRadius = UDim.new(1, 0)
    local accentGrad = Instance.new("UIGradient", contentAccent)
    ...
    createRotxDidacticHeader(contentArea)

    local pageHolder = Instance.new("Frame", contentArea)
    pageHolder.Size = UDim2.new(1, -10, 1, -55)
    pageHolder.Position = UDim2.new(0, 5, 0, 49)
    ...
    local function buildPage()
        ...
        p.ScrollBarImageColor3 = GUI_CYAN
    ...
    local function makeTopTab(label, idx, page)
        ...
        b.BackgroundColor3 = GUI_PANEL
        b.BackgroundTransparency = 0.46
        b.BorderSizePixel = 0
        b.Text = string.format("%02d  %s", idx, label)
        b.TextColor3 = GUI_WHITE
        b.Font = Enum.Font.GothamBold
        b.TextSize = label == "KEYBINDS" and 8 or 9
        b.AutoButtonColor = false
        b.LayoutOrder = idx
        Instance.new("UICorner", b).CornerRadius = UDim.new(0, 12)
        local tabGrad = Instance.new("UIGradient", b)
        tabGrad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, GUI_BG),
            ColorSequenceKeypoint.new(0.55, GUI_PURPLE_DEEP),
            ColorSequenceKeypoint.new(1, GUI_MAGENTA_DEEP)
        })
        tabGrad.Rotation = 28
        local tabScale = Instance.new("UIScale", b)
        tabScale.Scale = 0.98
    ...
    local function setActivePage(p)
        ...
        for _, t in ipairs(allTabs) do
            ...
            local tabScale = t.btn:FindFirstChildWhichIsA("UIScale")
            if tabScale then
                TS:Create(tabScale, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                    Scale = isActive and 1.06 or 0.98
                }):Play()
            end
        end
    end

    btnMain.BackgroundColor3 = GUI_PURPLE
    btnMain.BackgroundTransparency = 0
    btnMain.TextColor3 = GUI_WHITE
    local stMain = btnMain:FindFirstChildWhichIsA("UIStroke")
    if stMain then stMain.Color = GUI_BLACK; stMain.Transparency = 0.2 end
    local scaleMain = btnMain:FindFirstChildWhichIsA("UIScale")
    if scaleMain then scaleMain.Scale = 1.06 end
    ...
    local function mkSect(txt)
        ...
        line.BackgroundColor3 = GUI_BLACK
    ...
    local function mkRow(h)
        ...
        f.BackgroundColor3 = GUI_PANEL
        f.BackgroundTransparency = 0.12
        f.BorderSizePixel = 0
        f.LayoutOrder = #activePage:GetChildren() + 1
        Instance.new("UICorner", f).CornerRadius = UDim.new(0, 10)
        f.MouseEnter:Connect(function()
            TS:Create(f, TweenInfo.new(0.12), {BackgroundColor3 = GUI_PANEL_ALT}):Play()
        end)
        f.MouseLeave:Connect(function()
            TS:Create(f, TweenInfo.new(0.12), {BackgroundColor3 = GUI_PANEL}):Play()
        end)
        return f
    end
    ...
    local function animPill(pill, dot, on)
        TS:Create(pill,TweenInfo.new(0.18,Enum.EasingStyle.Quad),{BackgroundColor3=on and GUI_PURPLE or GUI_PANEL}):Play()
        TS:Create(dot,TweenInfo.new(0.18,Enum.EasingStyle.Back),{
            Position=on and UDim2.new(1,-21,0.5,-9) or UDim2.new(0,3,0.5,-9),
            BackgroundColor3=GUI_WHITE
        }):Play()
    end
    ...
    local function mkBox(parent, default, w, xOff, cb)
        ...
        tb.Focused:Connect(function() TS:Create(bs,TweenInfo.new(0.12),{Color=GUI_BLACK,Transparency=0}):Play() end)
        tb.FocusLost:Connect(function()
            TS:Create(bs,TweenInfo.new(0.12),{Color=GUI_BLACK,Transparency=0.28}):Play()
    ...
    do
        ...
        local function makeFOVBtn(val, x)
            ...
            btn.BackgroundColor3 = GUI_PURPLE_DARK
            btn.BorderSizePixel = 0
            ...
            if val == stretchFOV then
                btn.BackgroundColor3 = GUI_PURPLE_LIGHT
                btn.TextColor3 = GUI_WHITE
            end
            applyShimmerToText(btn, 0.7)
            btn.MouseButton1Click:Connect(function()
                ...
                for _, b in ipairs(btnFrame:GetChildren()) do
                    if b:IsA("TextButton") then
                        ...
                        if v == val then
                            b.BackgroundColor3 = GUI_PURPLE_LIGHT
                            b.TextColor3 = GUI_WHITE
                        else
                            b.BackgroundColor3 = GUI_PURPLE_DARK
                            b.TextColor3 = GUI_WHITE
                        end
                    end
                end
    ...
    do
        ...
        delBtn.BackgroundColor3 = GUI_PURPLE_DARK
        delBtn.BorderSizePixel = 0
        delBtn.Text = "DELETE SETTINGS"
        delBtn.TextColor3 = GUI_WHITE
        delBtn.Font = Enum.Font.GothamBold
        delBtn.TextSize = 14
        Instance.new("UICorner", delBtn).CornerRadius = UDim.new(0, 6)
        local delStroke = Instance.new("UIStroke", delBtn)
        delStroke.Color = GUI_BLACK
        ...
        delBtn.Activated:Connect(function()
            if deleteState==0 then
                deleteState=1
                delBtn.Text="CONFIRM?"
                delBtn.BackgroundColor3=GUI_PURPLE_LIGHT
                delBtn.TextColor3=GUI_WHITE
    ...
    pbFrame.BackgroundColor3 = GUI_PURPLE
    ...
    fillRegion.BackgroundColor3 = GUI_PURPLE_DARK
    ...
    progressFill.BackgroundColor3 = GUI_PURPLE_LIGHT
    progressFill.BorderSizePixel = 0
    Instance.new("UICorner", progressFill).CornerRadius = UDim.new(1, 0)
    local fillGrad = Instance.new("UIGradient", progressFill)
    fillGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, GUI_PURPLE),
        ColorSequenceKeypoint.new(0.5, GUI_PURPLE_LIGHT),
        ColorSequenceKeypoint.new(1, GUI_PURPLE)
    })
    ...
    progressPct.Text = "—"
    progressPct.TextColor3 = GUI_WHITE
    ...
    pbDiv.BackgroundColor3 = GUI_BLACK
    ...
    progressRadLbl.Text = "-- · --"
    progressRadLbl.TextColor3 = GUI_WHITE
    ...
    local function dragProgress(f)
        ...
        trackRotxConnection(UIS.InputChanged:Connect(function(i)
            if i == di and dn then
    ...
    local function setBarState(state, distance)
        ...
        if state == "STEALING" then
            TS:Create(stealLbl,   TweenInfo.new(0.2), {TextColor3 = GUI_WHITE}):Play()
            TS:Create(fillRegion, TweenInfo.new(0.2), {BackgroundColor3 = GUI_PURPLE}):Play()
        elseif state == "READY" then
            TS:Create(stealLbl,   TweenInfo.new(0.2), {TextColor3 = GUI_WHITE}):Play()
            TS:Create(fillRegion, TweenInfo.new(0.2), {BackgroundColor3 = GUI_PURPLE_LIGHT}):Play()
            if progressPct then
            ...
            TS:Create(stealLbl,   TweenInfo.new(0.2), {TextColor3 = GUI_WHITE}):Play()
            TS:Create(fillRegion, TweenInfo.new(0.2), {BackgroundColor3 = GUI_PURPLE_DARK}):Play()
            if progressPct and not isStealing then
                progressPct.Text = "—"
                progressPct.TextColor3 = GUI_WHITE
            end
    ...
    task.spawn(function()
        local stateFrame = pbFrame
        while stateFrame and stateFrame.Parent do
            task.wait(Steal.AutoStealEnabled and 0.25 or 1)
    ...
    task.spawn(function()
        ...
        while fpsLabel and fpsLabel.Parent do
            ...
            if elapsed > 0 then fpsAvg = frameCount / elapsed end
            frameCount = 0
            windowStart = now
            local ping = 0
            pcall(function()
            ...
            fpsLabel.Text = string.format("%d FPS | %dms", math.floor(fpsAvg + 0.5), math.floor(ping + 0.5))
        end
...
local function createMobilePanel()
    ...
    local ACCENT = GUI_WHITE
    local INACTIVE_BG = GUI_PANEL
    local INACTIVE_TEXT = GUI_WHITE
    local STROKE_COLOR = GUI_BLACK
    local ACTIVE_BG = GUI_PURPLE
    local ACTIVE_TEXT = GUI_WHITE
    ...
    local function createButton(name, text, order, isToggle, callback)
        ...
        local buttonGrad = Instance.new("UIGradient", btn)
        buttonGrad.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, GUI_BG),
            ColorSequenceKeypoint.new(0.58, GUI_PURPLE_DEEP),
            ColorSequenceKeypoint.new(1, GUI_MAGENTA_DEEP)
        })
        buttonGrad.Rotation = 35
        local stroke = Instance.new("UIStroke", btn)
        ...
        local function setActive(state, level)
            ...
            if active then
                ...
                buttonGrad.Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, GUI_PURPLE_DEEP),
                    ColorSequenceKeypoint.new(0.55, GUI_PURPLE),
                    ColorSequenceKeypoint.new(1, GUI_MAGENTA)
                })
...
local function createInstaResetFloatingButton()
    ...
    btnFrame.BackgroundColor3 = GUI_PANEL
    ...
    local function setActive(state)
        if state then
            btnFrame.BackgroundColor3 = GUI_PURPLE
            label.TextColor3 = GUI_WHITE
        else
            btnFrame.BackgroundColor3 = GUI_PANEL
            label.TextColor3 = GUI_WHITE
...
local function createBypassFloatingButton()
    ...
    btnFrame.BackgroundColor3 = bypassToggled and GUI_PURPLE or GUI_PANEL
    btnFrame.BackgroundTransparency = 0
    btnFrame.BorderSizePixel = 0
    btnFrame.ZIndex = 20
    Instance.new("UICorner", btnFrame).CornerRadius = UDim.new(0, 18)
...
trackRotxConnection(LP.CharacterAdded:Connect(function(char)
    stopAutoSteal()
    stopAutoLeft()
    stopAutoRight()
    stopBatCounter()
...
trackRotxConnection(UIS.InputBegan:Connect(function(input, gpe)
    if _anyKeyListening then return end
...
_G.ROTX_HUB_CLEANUP = function()
    local connections = {
        movementLoop, steppedConn, enemySpeedConn, stretchConn, stretchFovConn,
        antiRagdollConn, alConn, arConn, infJumpConn, holdJumpConn,
        holdJumpJumpConn, _aimbotConn, bypassConn, autoTPDownConn,
        antiLagDescConn, Conns.autoSteal, Conns.batCounter, Conns.progress
    }
    ...
    for _, connection in pairs(medusaResetConns) do
        if connection then pcall(function() connection:Disconnect() end) end
    end
    for _, item in ipairs(dropConnections) do
    ...
    for _, label in pairs(enemySpeedLabels) do
        if label and label.Parent then pcall(function() label.Parent:Destroy() end) end
    end
    if _G.ROTX_RUNTIME_CONNS then
        for _, connection in ipairs(_G.ROTX_RUNTIME_CONNS) do
            if connection then pcall(function() connection:Disconnect() end) end
    ...
    for _, screen in ipairs({gui, MobilePanel, instaResetFloatingButton, bypassFloatingButton}) do
...
task.spawn(function()
    local activeGui = gui
    while activeGui and activeGui.Parent do
        task.wait(30)
        if not activeGui.Parent then break end
        pcall(saveAllSettings)
    end
end)
